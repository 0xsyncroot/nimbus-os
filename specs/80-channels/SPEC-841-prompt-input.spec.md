...
- DELETE `src/onboard/keyPrompt.ts` post-migration (gap audit 3.9).
...
| T7 | Delete `keyPrompt.ts` | No import of `keyPrompt` remains in codebase; callers migrated to `PasswordPrompt` | 0 | T6 |
...
- `src/onboard/keyPrompt.ts` (DELETE after migration)
