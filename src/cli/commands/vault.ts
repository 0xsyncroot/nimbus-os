// vault.ts — `nimbus vault` subcommand (SPEC-505)
  ...
  if (!yes) {
    process.stdout.write(
      [
        ...
        '',
        'Run with --yes to confirm: nimbus vault reset --yes',
        'Pipe mode:  echo "sk-..." | nimbus vault reset --yes --key-stdin',
        '',
      ].join('\n'),
    );
    return 1;
...
export async function runVault(subArgs: string[]): Promise<number> {
  ...
  process.stdout.write(
    [
      '',
      'nimbus vault — manage the encrypted secrets vault',
      '',
      'Usage:',
      '  nimbus vault reset [--yes]                           Backup + clear vault + re-enter API key',
      '  nimbus vault reset --yes --key-stdin                 Read key from stdin pipe (non-TTY)',
      '  nimbus vault reset --yes --key-stdin --provider <n>  Override provider (anthropic|openai|groq|deepseek|ollama)',
      '  nimbus vault status                                   Show vault health (same as nimbus doctor)',
      '',
      'Examples:',
      '  echo "sk-..." | nimbus vault reset --yes --key-stdin',
      '  echo "sk-..." | nimbus vault reset --yes --key-stdin --provider openai',
      '',
    ].join('\n'),
  );
  return 0;
